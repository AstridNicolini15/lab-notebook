...
colors = [[(to_rgb('#b30a7bff'), 0.8),(to_rgb('#b30a7bff'), 0.2)], 
          [(to_rgb('#9e0004ff'), 0.8),(to_rgb('#9e0004ff'), 0.2)]
          ]
...
fig.suptitle('interval post = [1,2]')
ax_dict = fig.subplot_mosaic(mosaic)
special_dict = {'name' : '', 'ylims' : [0,1.05], 'title' : 'inclusion factor = 1.15 '}
...
for i, inclusion_factor in enumerate([2,3]) : 
    ...
    special_dict = {'name' : 'inclusion_factor-%i_correction_factor-0.7_' % inclusion_factor, 'ylims' : [0,1.05], 'title' : 'inclusion factor = %i ' % inclusion_factor}
...
for folder in folders : 
    ...
    for key in ['%s_contrast-1.0' % folder,
        ...
        for inclusion_factor in [1.15, 2, 3] : 
            ...
            #osi = [np.mean(T['selectivities'][T['significant_ROIs']]) for T in Tunings]
            osi = []
            for T in Tunings : 
...
SIs = compute_selectivities(Responses,
                          using='orth-resp', # or "fit"
                          angles=np.linspace(-22.5, 135, 8),
                          verbose=False)

SIs_fit = compute_selectivities(Responses,
                          using="fit",
                          angles=np.linspace(-22.5, 135, 8),
                          verbose=False)