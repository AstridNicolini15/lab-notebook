...
def compute_F_Fneu_traces(folderpath, neuropil_params, mode = 'return'):
    ...
    if mode == 'save' : #does F also change? => for some params yes?
            if not os.path.isdir(folderpath + '/traces/') : 
                os.mkdir(folderpath + '/traces/')

            filename = get_neuropil_params_filename(neuropil_params) 
            np.save(folderpath + '/traces/' + 'Fneu' + filename, Fneu)
            if neuropil_params['allow_overlap'] == True : 
                np.save(folderpath + '/traces/' + 'F' + filename, F)
