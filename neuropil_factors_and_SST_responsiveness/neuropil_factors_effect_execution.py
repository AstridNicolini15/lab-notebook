...
folders = [#"SST-cells_cond-GluN1-KO_Adult_V1",
        "SST-cells_WT_Adult_V1"]


summary_folder = '/home/user/DATA/Astrid/summary_neuropil_factor'
base_path = os.path.expanduser('~/DATA/Astrid/Cibele_data')

nMin_episodes = 2
nMIN_DATAFILES = 2

#neuropil_inclusion_factors = [0,0.5,1,1.15,1.3,1.5,2,3.5,6]
#neuropil_correction_factors = [0,0.15,0.3,0.55,0.7,0.85,1] 

neuropil_inclusion_factors = [2,3]
neuropil_correction_factors = [0.7]
...
#%% Create summaries with different neuropil factors
        ...
        if __name__=='__main__':
            ...
            for n in range(Nstart, Nend):
                ...
                if len(DATASET['files'][cond])>nMIN_DATAFILES:
                    ...
                    for r in range(nruns):
                        ...
                        for i in range(i0,imax):
                            proc = multiprocessing.Process(\
                                                target=process_file, 
                                                args=(DATASET['files'][cond][i], i, c, neuropil_inclusion_factor, neuropil_correction_factor))
                            procs.append(proc)
                            proc.start()
                    ...
                    # # saving data
                    np.save(os.path.join(summary_folder, 'withstd_' + str('corrfact_') + str(neuropil_correction_factor) + 'inclufact_' + str(neuropil_inclusion_factor) + 'Tunings_%s.npy' % c), Tunings)
